...
function llenarTabla(calificacionesBase) {
	for(clave in calificacionesBase) {
		var calificaciones = calificacionesBase[clave];

		var tbody = document.getElementById("cuerpotabla-calificacionesBase");
...
function alertContentsJSON() {
    if (http_request.readyState == 4) {
        if (http_request.status == 200) {
            var calificacionesBases = JSON.parse(http_request.response);
            llenarTabla(calificacionesBases);
        } else {
...
window.onload = function(){
	makeRequest('calificacionesBase.js','application/json');
}